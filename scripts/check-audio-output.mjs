...
assert.ok(audibleContextTime(ctx, 100) < firstBeat, 'Render clock several beats ahead must not start visuals');
assert.equal(audibleContextTime(ctx, 100), .994, 'Use output timestamp without subtracting device latency twice');
assert.ok(Math.abs(audibleContextTime(ctx, 120) - 1.014) < 1e-10, 'Interpolate output clock to the current animation frame');
assert.equal(audibleContextTime(ctx, 1000), audibleContextTime(ctx, 2000), 'Stalled output must not keep advancing through wall time');
assert.equal(audibleContextTime(ctx, 80), .994, 'Future timestamp cannot move animation backwards');
...
assert.ok(Math.abs(audibleContextTime(ctx, 100) - 3.694) < 1e-10, 'Prefer the estimate that accounts for more latency');
ctx.state = 'suspended';
...
const end = source.indexOf('  const stopLoop = useCallback(', start);
assert.ok(start >= 0 && end > start);
let outputTime = 0, nextFrame;
const flashes = [];
const audio = { context: {} }, engine = { current: audio };
const flashWhenAudible = vm.runInNewContext(ts.transpileModule(
  source.slice(start, end) + '\nflashWhenAudible;',
  { compilerOptions: { target: ts.ScriptTarget.ES2022 } },
).outputText, {
  useCallback: fn => fn, bolHits, engine,
  mounted: { current: true }, previewFrames: { current: new Set() },
...
for (const time of [1, 1.25, 1.5, 1.75]) { outputTime = time; nextFrame(); }
...
console.log('PASS: preview and phrase flashes follow actual output; audio reset cancels stale frames.');
